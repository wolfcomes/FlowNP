...
def is_3d_molecule(mol):
    """Check if a molecule is 3D by verifying that not all atoms lie in the same plane."""
    if mol is None or mol.GetNumAtoms() < 4:
        # Molecules with less than 4 atoms are always planar
        return False
    ...
    # The smallest singular value indicates how close the points are to lying on a plane
    # If it's very small (close to zero), the points are coplanar
    return s[2] > 1e-2  # Threshold can be adjusted
...
def process_split(split_indices, split_name, args, dataset_config, all_molecules, all_smiles):
    ...
    # --- 开始新增代码 ---
    print("Computing the marginal distribution of atomic radii, p(r)")
    # 假设特征提取器已经将每个分子的质心移动到原点 (0,0,0)
    # 计算每个原子位置向量的范数（即半径 r）
    # 利用 node_idx_array 来索引每个分子的原子
    centers = torch.zeros_like(all_positions)
    for i, (start_idx, end_idx) in enumerate(node_idx_array):
        mol_positions = all_positions[start_idx:end_idx]
        center = torch.mean(mol_positions, dim=0, keepdim=True)
        centers[start_idx:end_idx] = center

    # 将每个分子平移到原点
    centered_positions = all_positions - centers

    # 计算每个原子到其分子中心的距离（半径 r）
    all_r = torch.linalg.norm(centered_positions, dim=1)

    # 使用直方图来近似r的连续分布。箱数(bins)可以调整以获得不同粒度。
    n_r_bins = 1000  # 可以根据需要调整
    r_hist = torch.histogram(all_r, bins=n_r_bins, density=False) # 使用 density=False 获取计数
    
    # r_hist.hist 是每个箱子中的样本数, r_hist.bin_edges 是箱子的边界
    p_r_counts = r_hist.hist
    r_bin_edges = r_hist.bin_edges

    # 将计数归一化为概率分布
    p_r = p_r_counts.float() / p_r_counts.sum()
    # --- 结束新增代码 ---
    ...
    # --- 修改保存部分 ---
    # 将 p(a), p(e), p(c|a) 以及新增的 p(r) 相关信息保存到文件
    marginal_dists_file = output_dir / f'{split_name}_marginal_dists.pt'
    # 我们将 p_r (概率) 和 r_bin_edges (箱子边界) 一起保存，以便后续采样
    # 注意：保存的元组结构已改变，加载时需对应修改
    torch.save((p_a, p_c, p_e, p_c_given_a, p_r, r_bin_edges), marginal_dists_file)
...
def load_coconut_molecules(sdf_file, max_molecules=None):
    ...
    print(f"Non-3D molecules skipped: {non_3d_count}")
...
if __name__ == "__main__":
    ...
    sdf_file = raw_dir / 'coconut_sdf_3d-06-2025.sdf'  # Adjust filename as needed
