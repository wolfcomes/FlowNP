...
try:
    ...
    try:
        ...
        if os.path.exists(model_file):
            ...
            print(f"警告：NP模型文件不存在: {model_file}")
    except Exception as e:
        print(f"警告：加载NP模型时出错: {e}")
except ImportError:
...
def calculate_molecular_properties(smiles):
    ...
    if NP_MODEL is not None and scoreMol is not None:
        try:
            np_score = scoreMol(mol, NP_MODEL)
        except Exception as e:
            print(f"计算NP_score时出错: {e}")
            np_score = np.nan
...
def process_docking_file(file_path, group_name):
    ...
    properties = [p for p in properties if p is not None]  # 过滤None
    # 合并结果
...
def plot_property_distributions(combined_df, output_dir):
    ...
    # 过滤掉数据中不存在的性质
    available_properties = [prop for prop in properties if prop in combined_df.columns]
    
    # 调整子图布局以适应实际性质数量（3行3列）
        ...
        for group, data in combined_df.groupby('group'):
            ...
            if len(values) > 1:
                try:
                    ...
                    print(f"警告：无法为 {prop} ({group}) 计算密度曲线: {e}")
        
        ax.set_title(property_names.get(prop, prop))
        ax.set_xlabel(property_names.get(prop, prop))
        ax.set_ylabel('Density')
        ax.grid(True, linestyle='--', alpha=0.7)
        ax.legend()
    
    # 隐藏多余的子图
    for idx in range(n_props, len(axes_flat)):
