...
def plot_property_distributions(combined_df, output_dir):
    ...
    for i, prop in enumerate(row1_props):
        ...
        for source, group in valid_data.groupby('clean_source'):
            line = sns.kdeplot(group[prop], label=source, ax=ax, fill=False, linewidth=4, alpha=0.7)
            if i == 0:
                all_lines.append(line.get_lines()[-1])
                all_labels.append(source.split('.')[0])
        # ax.set_title(property_names_en[prop], fontsize=18)
        ax.set_ylabel('Density' if i == 0 else '', fontsize=20, fontweight='bold')
        ax.set_xlabel(property_names_en[prop], fontsize=20, fontweight='bold')
        
        # 设置坐标轴标尺
        ax.tick_params(axis='both', which='major', labelsize=16, length=6, width=0)
        # 如果需要进一步调整标尺密度，可以设置刻度间隔
        ax.xaxis.set_major_locator(plt.MaxNLocator(4))  # x轴大约显示5个刻度
        ax.yaxis.set_major_locator(plt.MaxNLocator(4))  # y轴大约显示5个刻度
    ...
    for i, prop in enumerate(row2_props):
        ...
        ax = axes[1, i]
        for source, group in valid_data.groupby('clean_source'):
            sns.kdeplot(group[prop], label=source, ax=ax, fill=False, linewidth=4, alpha=0.7)
    ...
    if 'SMILES' not in combined_df.columns:
        ...
        if not fingerprints:
            ...
            if perplexity_value > 1:
                ...
                # 设置坐标轴
                plt.title('t-SNE Visualization of Chemical Space (based on Morgan Fingerprints)', fontsize=28, fontweight='bold')
                ...
                # 设置刻度参数
                plt.xticks(fontsize=22)  # 增大x轴刻度字体
                plt.yticks(fontsize=22)  # 增大y轴刻度字体

                # 缩减刻度数量（可选，自动选择较少的刻度）
                plt.locator_params(axis='x', nbins=5)  # x轴只显示5个刻度
                plt.locator_params(axis='y', nbins=5)  # y轴只显示5个刻度

                plt.legend(bbox_to_anchor=(0.80, 1), loc='upper left', fontsize=22)
