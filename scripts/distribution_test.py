...
class SDFComparator:
    ...
    def read_sdf_directory(self, directory_path, max_molecules_per_file=1000):
        """读取目录下所有SDF文件并合并分子"""
        sdf_files = glob.glob(os.path.join(directory_path, "*.sdf"))
        if not sdf_files:
            print(f"在目录 {directory_path} 中未找到SDF文件")
            return []
        ...
        for sdf_file in sdf_files:
            ...
            molecules = self.read_sdf_file(sdf_file, max_molecules_per_file)
            all_molecules.extend(molecules)
            if len(all_molecules) >= max_molecules_per_file:
                all_molecules = all_molecules[:max_molecules_per_file]
                break
        
        print(f"从目录 {directory_path} 总共读取 {len(all_molecules)} 个分子")
        return all_molecules
    
    def load_molecules(self, input_path, sample_size=1000):
        """根据输入路径是文件还是目录来加载分子"""
        if os.path.isfile(input_path):
            return self.read_sdf_file(input_path, sample_size)
        elif os.path.isdir(input_path):
            return self.read_sdf_directory(input_path, sample_size)
        else:
    ...
    def compare_coordinates(self, coords1, coords2, label1="文件1", label2="文件2"):
        """比较两个坐标集的分布"""
        print("\n" + "="*50)
        print("坐标分布比较")
        print("="*50)
        ...
        # 基本统计信息
        print(f"{label1}坐标数量: {len(coords1)}")
        print(f"{label2}坐标数量: {len(coords2)}")
        ...
        for i, coord_name in enumerate(['X', 'Y', 'Z']):
            ...
            print(f"\n{coord_name}坐标比较:")
            print(f"  {label1} - 均值: {np.mean(coord1):.3f}, 标准差: {np.std(coord1):.3f}, 范围: [{np.min(coord1):.3f}, {np.max(coord1):.3f}]")
            print(f"  {label2} - 均值: {np.mean(coord2):.3f}, 标准差: {np.std(coord2):.3f}, 范围: [{np.min(coord2):.3f}, {np.max(coord2):.3f}]")
            
            # t检验
            t_stat, p_value = stats.ttest_ind(coord1, coord2)
            print(f"  t检验 p值: {p_value:.6f} ({'显著差异' if p_value < 0.05 else '无显著差异'})")
    
    def compare_atom_types(self, atoms1, atoms2, label1="文件1", label2="文件2"):
        """比较两个原子类型分布的分布"""
        print("\n" + "="*50)
        print("原子类型分布比较")
        print("="*50)
        ...
        print(f"{label1}原子总数: {len(atoms1)}")
        print(f"{label2}原子总数: {len(atoms2)}")
        ...
        for atom in sorted(all_atoms):
            ...
            comparison_data.append({
                ...
                f'{label1}_Freq(%)': f"{freq1:.2f}",
                f'{label2}_Freq(%)': f"{freq2:.2f}",
    ...
    def compare_bond_types(self, bonds1, bonds2, label1="文件1", label2="文件2"):
        """比较两个化学键类型分布的分布"""
        print("\n" + "="*50)
        print("化学键类型分布比较")
        print("="*50)
        ...
        print(f"{label1}化学键总数: {len(bonds1)}")
        print(f"{label2}化学键总数: {len(bonds2)}")
    ...
    def compare_sdf_files(self, file1, file2, sample_size=1000, label1="文件1", label2="文件2"):
        """主比较函数"""
        print(f"开始比较SDF文件/目录:")
        ...
        # 读取文件或目录
        print(f"\n加载 {label1}...")
        self.molecules1 = self.load_molecules(file1, sample_size)
        print(f"\n加载 {label2}...")
        self.molecules2 = self.load_molecules(file2, sample_size)
        ...
        # 进行比较
        self.compare_coordinates(coords1, coords2, label1, label2)
        atom_df = self.compare_atom_types(atoms1, atoms2, label1, label2)
        bond_df = self.compare_bond_types(bonds1, bonds2, label1, label2)
...
def main():
    """主函数"""
    # 设置命令行参数
    parser = argparse.ArgumentParser(description='比较两个SDF文件或目录的坐标、原子类型和化学键分布')
    parser.add_argument('--file1', type=str, default='data/coconut_raw/coconut_sdf_3d-06-2025.sdf', 
                       help='第一个SDF文件路径或目录 (默认: file1.sdf)')
    parser.add_argument('--file2', type=str, default='results/FlowNP.sdf',
                       help='第二个SDF文件路径或目录 (默认: file2.sdf)')
    parser.add_argument('--sample_size', type=int, default=1000,
                       help='从每个文件/目录中抽取的分子数量 (默认: 1000)')
    parser.add_argument('--label1', type=str, default='file1',
                       help='第一个输入的自定义标签 (默认: 文件1)')
    parser.add_argument('--label2', type=str, default='file2',
                       help='第二个输入的自定义标签 (默认: 文件2)')
    
    args = parser.parse_args()
    
    # 检查文件/目录是否存在
    if not os.path.exists(args.file1):
        print(f"错误: 路径 {args.file1} 不存在")
        print("请使用 --file1 参数指定正确的文件或目录路径")
        return
    
    if not os.path.exists(args.file2):
        print(f"错误: 路径 {args.file2} 不存在")
        print("请使用 --file2 参数指定正确的文件或目录路径")
        return
    
    # 创建比较器并执行比较
    comparator = SDFComparator()
    results = comparator.compare_sdf_files(
        args.file1, 
        args.file2, 
