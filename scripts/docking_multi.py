...
def process_sdf_file(sdf_path, receptor_dir, ref_ligand_dir,
    ...
    for mol_idx, mol in enumerate(suppl):
        ...
        try:
            ...
            logging.error(f"处理分子 {mol_idx} 时出错: {e}")
        finally:
            # 确保临时文件被删除
...
def batch_docking(ligand_dir, receptor_dir, ref_ligand_dir,
                  output_dir, data_name, num_poses=1, max_threads=144, mode="dock"):
    """批量处理所有SDF文件"""
    os.makedirs(output_dir, exist_ok=True)
    ...
    if all_results:
        ...
        csv_path = os.path.join(output_dir, f'{data_name}.csv')
...
if __name__ == "__main__":
    ligand_dir = "results/generation_results_CFG_1.5"   # 包含SDF文件的目录
    ...
    results = batch_docking(ligand_dir, receptor_dir, ref_ligand_dir,
                            output_dir, data_name, num_poses, max_threads=144, mode=mode)
