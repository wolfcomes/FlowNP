...
class MoleculeFeaturizer():
    ...
    def __init__(self, atom_map: str, n_cpus=1, max_atoms = 80, explicit_aromaticity=False):
        self.n_cpus = n_cpus
        ...
        if self.n_cpus == 1:
            for molecule in molecules:
                ...
                if positions is not None and self.max_atoms < positions.shape[0]:
                    positions, atom_types, atom_charges, bond_types, bond_idxs, bond_order_counts = None, None, None, None, None, None
...
def featurize_molecule(molecule: Chem.rdchem.Mol, atom_map_dict: Dict[str, int], explicit_hydrogens=True, max_atoms =None, explicit_aromaticity=False):
    ...
    bond_order_counts = torch.zeros(n_bond_orders, dtype=torch.int64)
