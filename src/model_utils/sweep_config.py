...
def register_hyperparameter_args(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    p.add_argument('--enable_dynamic_graph', type=str, default=None, help='use the sparse graph or not')
    p.add_argument('--knn_connectivity', type=int, default=None, help='degree of knn')
    p.add_argument('--sde', type=str, default=None, help='use sde or not')


    # mol-fm configs
    p.add_argument('--time_scaled_loss', type=str, default=None, help='whether to scale loss by time, true or false')
    p.add_argument('--exclude_charge', type=str, default=None, help='whether to exclude charge, true or false')
...
def merge_config_and_args(config: dict, args: argparse.Namespace) -> dict:
    ...
    for arg in ['n_vec_channels', 'n_hidden_scalars', 'n_hidden_edge_feats', 'n_recycles', 'n_molecule_updates', 'convs_per_update', 'n_cp_feats', 'n_message_gvps', 'n_update_gvps', 'rbf_dim', 'rbf_dmax', 'enable_dynamic_graph', 'knn_connectivity', 'sde']:
        if getattr(args, arg) is not None:
    ...
    if args.exclude_charge is not None:
        config['mol_fm']['exclude_charge'] = strtobool(args.exclude_charge)
