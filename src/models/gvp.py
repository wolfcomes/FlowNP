...
class GVP(nn.Module):
    def __init__(
        ...
        if vector_gating:
            # self.scalar_to_vector_gates = nn.Sequential(
            #     nn.LayerNorm(dim_feats_out),  # 关键：添加LayerNorm
            #     nn.Linear(dim_feats_out, dim_vectors_out)
            # )
...
class GVPConv(nn.Module):
    ...
    def __init__(self, scalar_size: int = 128, vector_size: int = 16, n_cp_feats: int = 0,
        ...
        elif isinstance(self.message_norm, (float, int)):
...
class HeteroGVPConv(nn.Module):
    """GVP graph convolution for heterogeneous graphs (配体-口袋交互)"""
    ...
    def __init__(self, 
                 ...
                 dropout: float = 0.0,
                 separate_params: bool = True,
                 scalar_size_pocket: int = 128,
                 vector_size_pocket: int = 16,
                 edge_feat_size_pocket: int = 0):
        """
        Args:
            node_types: 节点类型列表，如['ligand', 'pocket']
            edge_types: 边类型列表，如[('ligand','ll','ligand'), ('pocket','pp','pocket'), 
                                     ('ligand','lp','pocket'), ('pocket','pl','ligand')]
            separate_params: 是否为不同类型使用独立参数
        """
        super().__init__()
        ...
        self.scalar_size_pocket = scalar_size_pocket
        self.vector_size_pocket = vector_size_pocket
        self.edge_feat_size_pocket = edge_feat_size_pocket


        # 消息函数模块
        self.edge_message = nn.ModuleDict()
        for etype in edge_types:
            ...
            # 确定输入输出尺寸
            if src_type == 'pocket':
                src_scalar_size = self.scalar_size_pocket
                src_vector_size = self.vector_size_pocket
            else:
                src_scalar_size = self.scalar_size
                src_vector_size = self.vector_size
                
            if dst_type == 'pocket':
                dst_scalar_size = self.scalar_size_pocket
                dst_vector_size = self.vector_size_pocket
            else:
                dst_scalar_size = self.scalar_size
                dst_vector_size = self.vector_size
                
            # 确定边特征尺寸
            if 'pocket' in [src_type, dst_type]:
                edge_feat_size = self.edge_feat_size_pocket
            else:
                edge_feat_size = self.edge_feat_size


            for i in range(n_message_gvps):
                dim_vectors_in = src_vector_size
                dim_feats_in = src_scalar_size
                ...
                if i == 0:
                    ...
                    dim_vectors_in += 1  # 位移向量
                    dim_feats_in += rbf_dim + edge_feat_size
                    if use_dst_feats:
                        dim_vectors_in += dst_vector_size
                        dim_feats_in += dst_scalar_size
                else:
                    # 后续层：接收前一层的输出
                    dim_vectors_in = dst_vector_size  # 使用目标节点的向量维度
                    dim_feats_in = dst_scalar_size    # 使用目标节点的标量维度
                
                # 输出维度始终是目标节点的维度
                dim_vectors_out = dst_vector_size
                dim_feats_out = dst_scalar_size
            ...
            # 如果不需要独立参数，共享第一个边类型的参数
            if not separate_params and self.edge_message:
                self.edge_message[etype_str] = self.edge_message[next(iter(self.edge_message))]
            else:
                self.edge_message[etype_str] = nn.Sequential(*message_gvps)
        ...
        for ntype in node_types:
            ...
            if ntype == 'pocket':
                scalar_size = self.scalar_size_pocket
                vector_size = self.vector_size_pocket
            else:
            ...
            # 如果不需要独立参数，共享第一个节点类型的参数
            if not separate_params and self.node_update:
                self.node_update[ntype] = self.node_update[next(iter(self.node_update))]
            else:
                self.node_update[ntype] = nn.Sequential(*update_gvps)
        
        self.dropout = GVPDropout(self.dropout_rate)
        self.message_layer_norm = GVPLayerNorm(self.scalar_size)
        self.update_layer_norm = GVPLayerNorm(self.scalar_size)
        self.message_layer_norm_pocket = GVPLayerNorm(self.scalar_size_pocket)
        self.update_layer_norm_pocket = GVPLayerNorm(self.scalar_size_pocket)
        ...
        if isinstance(self.message_norm, str):
            if self.message_norm not in ['mean', 'sum']:
                raise ValueError(f"message_norm must be either 'mean', 'sum', or a number, got {self.message_norm}")
            self.agg_func = fn.mean if self.message_norm == 'mean' else fn.sum
        else:
            self.agg_func = fn.sum
    ...
    def forward(self, g: dgl.DGLGraph,
        ...
        Args:
            g: 异构图
            node_feats: 节点标量特征 {'ligand': ..., 'pocket': ...}
            coord_feats: 节点坐标 {'ligand': ..., 'pocket': ...}
            vec_feats: 节点向量特征 {'ligand': ..., 'pocket': ...}
            edge_feats: 边特征 {('ligand','ll','ligand'): ..., ...}
        """
        ...
        with g.local_scope():
            ...
            for etype in self.edge_types:
                if g.num_edges(etype) > 0:
                    etype_str = '_'.join(etype)
                    g.apply_edges(self._message_fn(etype_str), etype=etype)

            # 聚合消息到目标节点
            for ntype in self.node_types:
                if g.num_nodes(ntype) > 0:
                    # 收集所有指向该节点类型的边类型
                    incoming_etypes = [etype for etype in self.edge_types if etype[2] == ntype and g.num_edges(etype) > 0]
                    
                    if not incoming_etypes:
                        continue
                    
                    # --- 修改部分开始 ---
                    # 为标量和向量消息分别定义归约器
                    # self.agg_func 是在 __init__ 中定义的 fn.sum 或 fn.mean
                    # cross_reducer='sum' 用于聚合来自不同关系类型（如 'll' 和 'pl'）的消息
                    
                    # 1. 聚合标量消息
                    scalar_reducers = {
                        etype: (fn.copy_e('scalar_msg', 'm'), self.agg_func('m', 'scalar_msg'))
                        for etype in incoming_etypes
                    }
                    g.multi_update_all(scalar_reducers, 'sum')

                    # 2. 聚合向量消息
                    vector_reducers = {
                        etype: (fn.copy_e('vec_msg', 'm'), self.agg_func('m', 'vec_msg'))
                        for etype in incoming_etypes
                    }
                    g.multi_update_all(vector_reducers, 'sum')
                    # --- 修改部分结束 ---
                    ...
                    if isinstance(self.message_norm, (float, int)):
                        z = self.message_norm
                        # 确保消息存在才进行操作
                        if 'scalar_msg' in g.nodes[ntype].data:
                            g.nodes[ntype].data['scalar_msg'] = g.nodes[ntype].data['scalar_msg'] / z
                        if 'vec_msg' in g.nodes[ntype].data:
                            g.nodes[ntype].data['vec_msg'] = g.nodes[ntype].data['vec_msg'] / z
            ...
            for ntype in self.node_types:
                ...
                # 获取聚合后的消息
                scalar_msg = g.nodes[ntype].data.get('scalar_msg', torch.zeros_like(scalar_feats[ntype]))
                vec_msg = g.nodes[ntype].data.get('vec_msg', torch.zeros_like(vec_feats[ntype]))
                
                # dropout
                scalar_msg, vec_msg = self.dropout(scalar_msg, vec_msg)
                
                # 更新特征
                scalar_feat = scalar_feats[ntype] + scalar_msg
                vec_feat = vec_feats[ntype] + vec_msg
                
                # layer norm
                if ntype == 'pocket':
                    scalar_feat, vec_feat = self.message_layer_norm_pocket(scalar_feat, vec_feat)
                else:
                    scalar_feat, vec_feat = self.message_layer_norm(scalar_feat, vec_feat)

                # 节点更新函数
                scalar_residual, vec_residual = self.node_update[ntype]((scalar_feat, vec_feat))
                scalar_residual, vec_residual = self.dropout(scalar_residual, vec_residual)
                
                # 残差连接
                new_scalar = scalar_feat + scalar_residual
                new_vector = vec_feat + vec_residual
                
                # 最终layer norm
                if ntype == 'pocket':
                    new_scalar, new_vector = self.update_layer_norm_pocket(new_scalar, new_vector)
                else:
                    new_scalar, new_vector = self.update_layer_norm(new_scalar, new_vector)
                
                new_node_feats[ntype] = new_scalar
                new_vec_feats[ntype] = new_vector

        return new_node_feats, new_vec_feats

    def _message_fn(self, etype_str):
        """为特定边类型创建消息函数"""
        def message_func(edges):
            ...
            # 拼接标量特征
            scalar_feats = [edges.src['h'], edges.data['d']]
            if self.edge_feat_size > 0 and 'a' in edges.data:
                scalar_feats.append(edges.data['a'])
            if self.use_dst_feats:
                scalar_feats.append(edges.dst['h'])
            scalar_feats = torch.cat(scalar_feats, dim=1)
            ...
            # 通过消息函数
            scalar_msg, vector_msg = self.edge_message[etype_str]((scalar_feats, vec_feats))
            
            return {"scalar_msg": scalar_msg, "vec_msg": vector_msg}
        
        return message_func

class GVPAttentionConv(nn.Module):
    """GVP graph convolution layer with an attention mechanism."""

    def __init__(self, scalar_size: int = 128, vector_size: int = 16, n_cp_feats: int = 0,
                  scalar_activation=nn.SiLU, vector_activation=nn.Sigmoid,
                  n_message_gvps: int = 1, n_update_gvps: int = 1,
                  use_dst_feats: bool = False, rbf_dmax: float = 20, rbf_dim: int = 16,
                  edge_feat_size: int = 0, dropout: float = 0.0):
        ...
        # --- 计算消息函数的输入维度 ---
        # 矢量部分
        dim_vectors_in_msg = vector_size + 1 # 源节点矢量 + 相对位移矢量
        # 标量部分
        dim_feats_in_msg = scalar_size + rbf_dim + edge_feat_size # 源节点标量 + RBF距离 + 边特征
        
        if use_dst_feats:
            dim_vectors_in_msg += vector_size
            dim_feats_in_msg += scalar_size
        ...
        message_gvps = []
        for i in range(n_message_gvps):
            dim_vectors_in = dim_vectors_in_msg if i == 0 else vector_size
            dim_feats_in = dim_feats_in_msg if i == 0 else scalar_size
        ...
        # 2. NEW: 注意力计算网络
        # 该网络接收与消息函数相同的标量输入，并输出一个原始注意力分数
        self.attention_net = nn.Sequential(
            nn.Linear(dim_feats_in_msg, 1),
            nn.LeakyReLU(0.2)
        )
    ...
    def forward(self, g: dgl.DGLGraph, 
        ...
        with g.local_scope():
            ...
            # MODIFIED: 在所有边上计算消息和原始注意力分数
            g.apply_edges(self.message)

            # NEW: 使用 edge_softmax 对注意力分数进行归一化
            g.edata['alpha'] = dgl.ops.edge_softmax(g, g.edata['e'])

            # NEW: 使用注意力权重对消息进行加权
            g.edata['scalar_msg_w'] = g.edata['scalar_msg'] * g.edata['alpha']
            # 注意力权重 alpha 是标量，可以直接广播到矢量消息上
            g.edata['vec_msg_w'] = g.edata['vec_msg'] * g.edata['alpha'].unsqueeze(-1)

            # MODIFIED: 聚合加权后的消息
            g.update_all(fn.copy_e("scalar_msg_w", "m"), fn.sum("m", "scalar_msg"))
            g.update_all(fn.copy_e("vec_msg_w", "m"), fn.sum("m", "vec_msg"))
    ...
    # MODIFIED: message 函数现在还需要计算注意力分数
    def message(self, edges):
        # --- 消息计算的输入准备 (与 GVPConv 相同) ---
        vec_feats = [edges.data["x_diff"].unsqueeze(1), edges.src["v"]]
        if self.use_dst_feats:
            vec_feats.append(edges.dst["v"])
        vec_feats = torch.cat(vec_feats, dim=1)
        ...
        # 2. NEW: 计算原始注意力分数 `e`
        # 我们使用与消息计算相同的标量输入 `scalar_feats`
        attention_score = self.attention_net(scalar_feats)

        return {
            "scalar_msg": scalar_message, 
            "vec_msg": vector_message,
            "e": attention_score  # 返回原始注意力分数
        }
