...
class CTMCVectorField(nn.Module):
    ...
    def __init__(self, n_atom_types: int,
                    ...
                    # cat_temperature_schedule: str = 'decay',
                    cat_temp_decay_max: float = 2.0,
                    cat_temp_decay_a: float = 0.3,
    ...
    # def sample_conditional_path(self, g, t, node_batch_idx, edge_batch_idx, upper_edge_mask,marginal_dists_file):
    #     """
    #     通过最优传输对齐节点后，对条件路径 p(g_t|g_0,g_1) 进行SDE采样。
    #     噪声仅应用于连续坐标'x'。
    #     离散特征采用 t*特征 + (1-t)*掩码列 的混合形式。
    #     """
    #     _, alpha_t = self.interpolant_scheduler.interpolant_weights(t)
    #     device = g.device

    #     try:
    #         p_a, p_c, p_e, p_c_given_a = torch.load(marginal_dists_file,map_location=device)
    #     except:
    #         p_a, p_c, p_e, p_c_given_a, p_r, r_bin_edges = torch.load(marginal_dists_file,map_location=device)
    ...
    #     # 2. 处理离散节点特征 a, c (改为混合形式)
    #     for feat, feat_idx in zip(['a', 'c'], [1, 2]):
    #         if self.mask_idxs[feat] == 0:
    #             continue
            
    #         alpha_t_feat = alpha_t[:, feat_idx][node_batch_idx].unsqueeze(-1)  # 形状: [num_nodes, 1]
    #         # 获取目标特征和对应的alpha值
    #         feat_1_true = g.ndata[f'{feat}_1_true']

    #         if feat == 'a':
    #             feat_1_true = feat_1_true * alpha_t_feat + (1 - alpha_t_feat) * p_a.repeat(feat_1_true.shape[0], 1)
    #         elif feat == 'c':
    #             feat_1_true = feat_1_true * alpha_t_feat + (1 - alpha_t_feat) * p_c.repeat(feat_1_true.shape[0], 1)
            
    #         feat_1_true = torch.cat([feat_1_true, torch.zeros(feat_1_true.shape[0], 1, device=device)], dim=1)
            
            
    #         # feat_1_true = one_hot(Categorical(feat_1_true).sample(), num_classes=self.n_cat_feats[feat]+1).float()

            
    #         # 创建掩码向量
    #         mask_vector = torch.zeros_like(feat_1_true)
    #         mask_vector[:, self.mask_idxs[feat]] = 1.0
            
    #         # 计算混合特征: t * 目标特征 + (1-t) * 掩码列
    #         mixed_feat = alpha_t_feat * feat_1_true + (1 - alpha_t_feat) * mask_vector
            
    #         g.ndata[f'{feat}_t'] = mixed_feat
    ...
    #     num_edges = int(g.num_edges() // 2)
    #     e_idx = self.canonical_feat_order.index('e')
    #     alpha_t_e = alpha_t[:, e_idx][edge_batch_idx][upper_edge_mask].unsqueeze(-1)  # 形状: [num_upper_edges, 1]


        
    #     # 获取上三角边的目标特征
    #     e_1_true_upper = g.edata['e_1_true'][upper_edge_mask]
    #     e_1_true_upper = e_1_true_upper * alpha_t_e + (1 - alpha_t_e) * p_e.repeat(e_1_true_upper.shape[0], 1)
    #     e_1_true_upper = torch.cat([e_1_true_upper, torch.zeros(e_1_true_upper.shape[0], 1, device=device)], dim=1)
    #     # e_1_true_upper = one_hot(Categorical(e_1_true_upper).sample(), num_classes=self.n_cat_feats['e']+1).float()
    #     # e_1_true_upper = torch.cat([e_1_true_upper, torch.zeros(e_1_true_upper.shape[0], 1, device=device)], dim=1)  # 形状: [num_upper_edges, num_classes]

    #     # 创建掩码向量
    #     mask_vector_e = torch.zeros_like(e_1_true_upper)
    #     mask_vector_e[:, self.mask_idxs['e']] = 1.0
        
    #     # 计算混合特征: t * 目标特征 + (1-t) * 掩码列
    #     mixed_e_upper = alpha_t_e * e_1_true_upper + (1 - alpha_t_e) * mask_vector_e
        
    #     # 构建完整的边特征矩阵
    #     n, d = g.edata['e_1_true'].shape
    #     e_t = torch.zeros((n, d+1), dtype=g.edata['e_1_true'].dtype, device=g.device)
    #     e_t[upper_edge_mask] = mixed_e_upper
    #     e_t[~upper_edge_mask] = mixed_e_upper  # 下三角部分与上三角相同
    ...
    def forward(self, g: dgl.DGLGraph, t: torch.Tensor,
                 node_batch_idx: torch.Tensor, edge_batch_idx: torch.Tensor, upper_edge_mask: torch.Tensor, apply_softmax=False, remove_com=False):
        ...
        with g.local_scope():
            ...
            batch_size = node_scalar_features.shape[0]
            # rni_dim = 8
            # if self.training:
            #     rni = torch.randn(batch_size, rni_dim, device=device) * 0.05
            # else:
            #     torch.manual_seed(42)
            # rni = torch.randn(batch_size, rni_dim, device=device)* 0.1
            
            # node_scalar_features = torch.cat([node_scalar_features, rni], dim=-1)
    ...
    def denoise_graph(self, g: dgl.DGLGraph,
        ...
        x_diff, d = precompute_distances(g, rbf_dmax = self.rbf_dmax, rbf_dim = self.rbf_dim)
        for recycle_idx in range(self.n_recycles):
            for conv_idx, conv in enumerate(self.conv_layers):
                ...
                if conv_idx != 0 and (conv_idx + 1) % self.convs_per_update == 0:
                    ...
                    x_diff, d = precompute_distances(g, node_positions, rbf_dmax = self.rbf_dmax, rbf_dim = self.rbf_dim)
    ...
    def step(self, g: dgl.DGLGraph, s_i: torch.Tensor, t_i: torch.Tensor,
        ...
        for feat_idx, feat in enumerate(self.canonical_feat_order):
            ...
            if feat == 'e':
                p_t = torch.cat([p_s_1, torch.zeros(data_src[f'{feat}_t'].shape[0]//2, 1, device=device)], dim=1)
            else:
                p_t = torch.cat([p_s_1, torch.zeros(data_src[f'{feat}_t'].shape[0], 1, device=device)], dim=1)
            ...
            if feat == 'e':
                ...
                e_logits = torch.zeros_like(g.edata['e_t'])[:, :-1]
                e_logits[upper_edge_mask] = p_s_1
                e_logits[~upper_edge_mask] = p_s_1
                p_s_1 = e_logits

            #     e_logits = torch.zeros_like(g.edata['e_t'])
            #     e_logits[upper_edge_mask] = p_t
            #     e_logits[~upper_edge_mask] = p_t
            #     p_t = e_logits
            ...
            # if t_i == 1.0:
            #     data_src[f'{feat}_t'] = one_hot(Categorical(p_t).sample().long(), num_classes=self.n_cat_feats[feat]+1).float()
            # else:
            #     data_src[f'{feat}_t'] = p_t

            data_src[f'{feat}_t'] = xt
            data_src[f'{feat}_1_pred'] = x_1_sampled
            data_src[f'{feat}_logits'] = p_s_1
        
        if self.enable_dynamic_graph and t_i < 0.98:
            if frag_features is not None:
                g, upper_edge_mask, edge_batch_idx = reconstruct_graph_with_frag(g, upper_edge_mask, node_batch_idx, frag_features, t=t_i, k=self.knn_connectivity)
            else:
                g, upper_edge_mask, edge_batch_idx = reconstruct_graph_dynamic(g, upper_edge_mask, node_batch_idx, k=self.knn_connectivity)
...
class ContextualCTMCVectorField(CTMCVectorField):
    ...
    def forward(self, g: dgl.DGLGraph, 
        ...
        if no_pocket:
            ...
            hetero_g.remove_edges(hetero_g.edges(form='eid',etype=('ligand', 'lp', 'pocket')), etype=('ligand', 'lp', 'pocket'))
            hetero_g.remove_edges(hetero_g.edges(form='eid',etype=('pocket', 'pl', 'ligand')), etype=('pocket', 'pl', 'ligand'))
            hetero_g.remove_edges(hetero_g.edges(form='eid',etype=('pocket', 'pp', 'pocket')), etype=('pocket', 'pp', 'pocket'))
            hetero_g.remove_nodes(hetero_g.nodes(ntype='pocket'), ntype='pocket')
        ...
        with g.local_scope(), pocket_g.local_scope():
            ...
            # 这里是由于heterograph的边索引和g的边索引不一致，所以需要重新排序
            src, dst = g.edges()
            src_hetero, dst_hetero = hetero_g.edges(etype=('ligand','ll','ligand'))
            edges_g_set = set(zip(src.tolist(), dst.tolist()))
            edges_hetero_set = set(zip(src_hetero.tolist(), dst_hetero.tolist()))
            edge_to_idx = {}
            for idx, (s, d) in enumerate(zip(src.tolist(), dst.tolist())):
                edge_to_idx[(s, d)] = idx
            reorder_indices = torch.zeros(g.num_edges(), dtype=torch.long, device=device)
            for i, (s, d) in enumerate(zip(src_hetero.tolist(), dst_hetero.tolist())):
                reorder_indices[i] = edge_to_idx[(s, d)]
            num_nodes_per_graph = g.batch_num_nodes()
            batch_size = g.batch_size
            g = dgl.reorder_graph(g, node_permute_algo=None, edge_permute_algo='custom', permute_config={'edges_perm': reorder_indices}, store_ids=False)
            src, dst = g.edges()
            g.set_batch_num_nodes(num_nodes_per_graph)
            g.set_batch_num_edges(torch.bincount(node_batch_idx[src], minlength=batch_size))
            ...
            for recycle_idx in range(self.n_recycles):
                for conv_idx, conv in enumerate(self.hetero_conv_layers):
                    ...
                    if conv_idx != 0 and (conv_idx + 1) % self.convs_per_update == 0:
                        ...
                        x_diff, d = precompute_distances(g, node_positions['ligand'], rbf_dmax=self.rbf_dmax, rbf_dim=self.rbf_dim)
                        edge_features[('ligand','ll','ligand')] = self.edge_updaters[updater_idx](
                            g, node_scalar_features['ligand'], edge_features[('ligand','ll','ligand')], d=d
                        )

            node_scalar_features = node_scalar_features['ligand']
            node_positions = node_positions['ligand']

            # Here we sort the edge features in g according to the sorted_indices
            reverse_indices = torch.zeros(g.num_edges(), dtype=torch.long, device=device)
            for i,j in enumerate(reorder_indices):
                reverse_indices[j] = i
            edge_features = edge_features[('ligand','ll','ligand')][reverse_indices]
    ...
    def step(self, g: dgl.DGLGraph, pocket_g: dgl.DGLGraph, s_i: torch.Tensor, t_i: torch.Tensor,
        ...
        x_1_cond = dst_dict['x']
        x_1_uncond = dst_dict_no_pocket['x']
        
        # CFG
        x_1 = x_1_uncond + cfg_scale * (x_1_cond - x_1_uncond)
        ...
        for feat_idx, feat in enumerate(self.canonical_feat_order):
            ...
            p_s_1_no_pocket = dst_dict_no_pocket[feat]
            p_s_1_no_pocket = F.softmax(torch.log(p_s_1_no_pocket)/temperature, dim=-1) # log probabilities

            if dfm_type == 'campbell':


                xt, x_1_sampled = \
                campbell_step_pocket(p_1_given_t=p_s_1, 
                                p_1_given_t_no_pocket=p_s_1_no_pocket,
                                xt=xt, 
                                stochasticity=eta, 
                                hc_thresh=hc_thresh, 
                                alpha_t=alpha_t_i[feat_idx], 
