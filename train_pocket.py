...
from src.model_utils.sweep_config import merge_config_and_args, register_hyperparameter_args
from src.model_utils.load_pocket import read_config_file, model_from_config, data_module_from_config
...
if __name__ == "__main__":
    ...
    # create data module
    data_module: PocketLigandDataModule = data_module_from_config(config)

    # create model
    model: PocketFlowMol = model_from_config(config, seed_ckpt=args.seed_model)
    ...
    trainer = pl.Trainer(logger=None, **trainer_config, callbacks=[checkpoint_callback, pbar_callback])
